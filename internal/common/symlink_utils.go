...
// `err`
func GetSymLinkSourceAndTarget(dev diskutils.BlockDevice, symlinkDir, existingSymlink string) (string, string, bool, error) {
	var source string
	var target string
	var idExists = true
	var err error
	...
	if errors.As(err, &diskutils.IDPathNotFoundError{}) {
		// no disk-by-id
		idExists = false
		source = devLabelPath
	} else if err != nil {
